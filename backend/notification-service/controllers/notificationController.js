...
exports.getUserNotifications = async (req, res) => {
  try {
      ...
      notifications.map(async (notification) => {
        try {
          const postResponse = await axios.get(
            `http://proxy/api/posts/${notification.postId}`,
            {
              headers: {
          ...
          console.error(
            `Error fetching post ${notification.postId}:`,
            error.message
          );
    ...
    res
      .status(200)
      .json({ success: true, notifications: notificationsWithPosts });
  } catch (error) {
  ...
  try {
    ...
    if (notification.expiresAt < currentTime) {
      ...
      return res
        ...
        .json({
          success: true,
          message: "Expired notification deleted after being marked as seen.",
        });
    }
...
exports.createNotification = async (req, res) => {
  try {
    ...
    const response = await axios.get("http://proxy/api/auth", {
      headers: {
    ...
    if (!users.length) {
      return res
        .status(404)
        .json({ success: false, message: "No users to notify." });
    }
